...
type Props = {
  ...
  showCotacaoLink?: boolean;
};

export function PublicListingView({ listing, showCotacaoLink = true }: Props) {
  const hero = listing.midia.capa || listing.midia.fotos[0] || null;
  const where = [listing.localizacao.bairro, listing.localizacao.cidade, listing.localizacao.uf]
    .filter(Boolean)
    .join(', ');

  const cotacaoHref =
    listing.hotelId != null && listing.hotelId !== ''
    ...
    <main className="min-h-screen bg-stone-50 text-stone-900">
      {listing.isPreview ? (
        <div
          ...
          role="status"
        >
          Pré-visualização — não é a página pública publicada
        </div>
      ) : null}
      ...
      <div className="mx-auto grid max-w-5xl gap-8 px-4 py-10 md:grid-cols-[1.4fr_0.8fr] md:px-6">
        ...
        <aside className="h-fit rounded-2xl border border-stone-200 bg-white p-5 shadow-sm">
          ...
          {showCotacaoLink && !listing.isPreview ? (
            <Link
              href={cotacaoHref}
            ...
            <p className="mt-6 rounded-xl bg-stone-100 px-4 py-3 text-center text-sm text-stone-600">
              Cotação indisponível em pré-visualização
            </p>
          ) : null}
